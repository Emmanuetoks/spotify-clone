...
const EditModal = () => {
  const playlistInView: TPlaylistSearchParam = useParams();
  const [playlists, setPlayLists] = usePlayLists().libraryPlaylists;
  const { register, handleSubmit } = useForm<UpdateInput>();

  // To get the playlist name, i will use react query to fetch the data in order to get the playlist

  const { closeModal, openModal, modalOpen } = useEditModalState();
  const onSubmit: SubmitHandler<UpdateInput> = (update) => {
    setPlayLists({
      type: "update",
      payload: { ...update, id: playlistInView.playlistID as string },
    });
    closeModal();
  };

  const pS = playlists?.find((el) => el.id === playlistInView.playlistID);
  return (
